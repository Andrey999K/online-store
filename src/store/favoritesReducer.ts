...
type Action =
  | { type: "ADD_IN_FAVORITES"; payload: Product }
  | { type: "DELETE_FROM_FAVORITES"; payload: Product }
  ...
  switch (action.type) {
    case "ADD_IN_FAVORITES":
      if (localStorage.getItem("products_in_favorites")) {
        const massProduct = JSON.parse(
          localStorage.getItem("products_in_favorites") || "[]"
        );
        massProduct.push(action.payload);
        localStorage.setItem(
          "products_in_favorites",
          JSON.stringify(massProduct)
        );
      } else {
        localStorage.setItem(
          "products_in_favorites",
    ...
    case "DELETE_FROM_FAVORITES":
      if (localStorage.getItem("products_in_favorites")) {
        let massProducts: Product[] = JSON.parse(
          localStorage.getItem("products_in_favorites") || "[]"
        );
        massProducts = massProducts.filter(
          item => item.id !== action.payload.id
        );
        localStorage.setItem(
          "products_in_favorites",
          JSON.stringify(massProducts)
        );
      }
      return {
        ...state,
